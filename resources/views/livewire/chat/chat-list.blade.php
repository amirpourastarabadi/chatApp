        ...
        <ul class="p-2 grid w-full spacey-y-2">
            ...
            <li id="conversation-{{ $conversation?->id }}" wire:key="{{ $conversation?->id }}" class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 {{$conversation->id == $selectedConversation?->id ? 'bg-gray-400/70' : ''}}">
                <a href="#" class="shrink-0">
                    <x-avatar src="https://source.unsplash.com/500x500?face-{{$conversation?->receiver?->id}}" />
                </a>
                <aside class="grid grid-cols-12 w-full">
                    <a href="{{route('chats.show', ['chat' => $conversation?->id])}}" class="col-span-11 border-b pb-2 border-gray-200 relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
                        {{-- name and date  --}}
                        <div class="flex justify-between w-full items-center">
                            <h6 class="truncate font-medium tracking-wider text-gray-900">
                                {{ $conversation->reciver?->name}}
                            </h6>
                        ...
                        <div class="flex gap-x-2 items-center">
                            ...
                            @if ($conversation->receiverReadLastMessage())
                            <span>
                        ...
                        <x-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                <div class="w-full p-1">
                                    ...
                                    <button onclick="confirm('Are you sure?')||event.stopImmediatePropagation()" class="items-center gap-3 flex w-full px-4 py-2 text-left text-sm leading-5 text-gray-500 hover:bg-gray-100 transition-all duration-150 ease-in-out focus:outline-none focus:bg-gray-100">
                                        <span>
